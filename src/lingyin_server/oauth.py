...
OAUTH_SCOPES = ["lingyin", "mcp:tools", "claudeai"]
...
class LingYinRefreshToken(RefreshToken):
    resource: str | None = None
    family_id: str


class LingYinOAuthProvider(
    OAuthAuthorizationServerProvider[AuthorizationCode, LingYinRefreshToken, AccessToken]
):
    ...
    async def authorize(self, client: OAuthClientInformationFull, params: AuthorizationParams) -> str:
        ...
        pending = {
            ...
            "state": params.state,
            "scopes": params.scopes or ["lingyin"],
    ...
    def _store_token_pair(
        ...
        access = AccessToken(
            ...
            subject=subject or "lingyin-owner",
            claims={"iss": self.issuer_url, "family_id": family_id},
        )
        refresh = LingYinRefreshToken(
            token=raw_refresh,
            client_id=client_id,
            scopes=scopes,
    ...
    async def load_refresh_token(
        ...
        if not row:
            return None
        payload = json.loads(row["payload_json"])
        return LingYinRefreshToken(token=refresh_token, **payload)

    async def exchange_refresh_token(
        self,
        client: OAuthClientInformationFull,
        refresh_token: LingYinRefreshToken,
        scopes: list[str],
    ) -> OAuthToken:
        with self._lock, self._connect() as db:
            deleted = db.execute(
            ...
            return self._store_token_pair(
                ...
                list(dict.fromkeys(["lingyin", *scopes])),
    ...
    async def load_access_token(self, token: str) -> AccessToken | None:
        if self._password_matches(token, self.owner_password):
            return AccessToken(
                token=token,
                client_id="lingyin-static-owner",
                scopes=OAUTH_SCOPES,
    ...
    async def revoke_token(self, token: AccessToken | LingYinRefreshToken) -> None:
...
def render_oauth_login(request_id: str, pending: dict | None, error: str = "") -> str:
    if not pending:
        ...
        <h1>让 {client_name} 连接聆音？</h1>
        <p>输入你在 Zeabur 设置的 <code>LINGYIN_ACCESS_TOKEN</code>。密码只交给聆音，不会发送给 Claude。</p>
        {error_html}
        <form method="post" action="/oauth/login">
          <input type="hidden" name="request" value="{safe_request}">
          <label for="password">聆音访问密码</label>
