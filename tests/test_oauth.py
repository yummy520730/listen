...
def _oauth_app(provider: LingYinOAuthProvider):
    server = FastMCP(
        "OAuth test",
        stateless_http=True,
        json_response=True,
...
async def test_oauth_dcr_pkce_and_refresh_flow(tmp_path):
    base_url = "https://listen.example"
    provider = LingYinOAuthProvider(
    ...
    async with httpx.AsyncClient(transport=transport, base_url=base_url, follow_redirects=False) as client:
        ...
        assert "lingyin" in first_tokens["scope"].split()
